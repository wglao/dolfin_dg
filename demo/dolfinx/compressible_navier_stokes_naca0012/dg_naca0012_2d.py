...
import dolfin_dg as dfdg
from dolfin_dg.dolfinx import dwr, mark
...
u_ref = fem.Constant(mesh, M_0*c_0)
rho_in = fem.Constant(mesh, rho_0)
u_in = u_ref * n_in

# The initial guess used in the Newton solver. Here we use the inlet flow.
rhoE_in_guess = dfdg.aero.energy_density(p_0, rho_in, u_in, gamma)
...
for ref_level in range(n_ref_max):
    ...
    fts = dfx.mesh.meshtags(mesh, mesh.topology.dim-1, bdry_facets, values)

    ds = ufl.Measure('ds', domain=mesh, subdomain_data=fts)

    # Problem function space, (rho, rho*u1, rho*u2, rho*E)
    V = fem.functionspace(mesh, ("DG", poly_o, (4,)))
    ...
    inflow = dfdg.aero.subsonic_inflow(rho_in, u_in, u_vec, gamma)
    no_slip_bc = dfdg.aero.no_slip(u_vec)
    outflow = dfdg.aero.subsonic_outflow(p_0, u_vec, gamma)

    # Assemble these conditions into DG BCs
    bcs = [dfdg.DGDirichletBC(ds(INLET), inflow),
           dfdg.DGAdiabticWallBC(ds(WALL), no_slip_bc),
           dfdg.DGDirichletBC(ds(OUTLET), outflow)]
    ...
    ce = dfdg.CompressibleNavierStokesOperator(mesh, V, bcs, mu=1.0/Re)
    F = ce.generate_fem_formulation(u_vec, v_vec, h_measure=h, c_ip=20.0)
    J = ufl.derivative(F, u_vec)

    # Set up the problem and solve
    problem = fem.petsc.NonlinearProblem(F, u_vec, J=J)
    ...
    rho, u, E = dfdg.aero.flow_variables(u_vec)
    p = dfdg.aero.pressure(u_vec, gamma)
    l_ref = fem.Constant(mesh, 1.0)
    ...
    sigma = fem.Constant(mesh, 20.0)*fem.Constant(mesh, float(max(poly_o**2, 1)))/h
    import dolfin_dg.primal.aero as prim_aero
    F_v_adiabatic = prim_aero.compressible_navier_stokes_adiabatic_wall(
        u_vec, v_vec, gamma=gamma, mu=1/Re).F_vec[1]
    G_adiabitic = dfdg.math.homogeneity_tensor(F_v_adiabatic, u_vec)
    ...
    drag += ufl.inner(
        sigma*dfdg.math.hyper_tensor_product(G_adiabitic, ufl.outer(u_vec - no_slip_bc, n)),
        ufl.outer(z_drag, n))*ds(WALL)

    # The lift coefficient
    psi_lift = fem.Constant(
    ...
    drag_val = mesh.comm.allreduce(
        fem.assemble_scalar(fem.form(drag)), MPI.SUM)
    lift_val = mesh.comm.allreduce(
        fem.assemble_scalar(fem.form(lift)), MPI.SUM)
    ...
    if ref_level < n_ref_max - 1:
        V_star = fem.functionspace(mesh, ('DG', poly_o+1, (4,)))
        ...
        est = dwr.NonlinearAPosterioriEstimator(
            J, F, drag, u_vec, V_star)
        indicators = est.compute_indicators(petsc_options={
            "ksp_type": "preonly",
            "pc_type": "lu",
            "pc_factor_mat_solver_type": "mumps"})
        cell_markers = mark.maximal_indices_fraction(
            indicators, 0.1)

        info("Refining mesh")
        edges_to_ref = dfx.mesh.compute_incident_entities(
            mesh.topology, cell_markers, mesh.topology.dim, 1)
        new_mesh, _, _ = dfx.cpp.refinement.refine_plaza(
            mesh._cpp_object, edges_to_ref, True,
            dfx.mesh.RefinementOption.none)
        new_mesh = dfx.mesh.Mesh(
